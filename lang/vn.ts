export default {
  home: {
    name: "Nguyễn Tiến Hoạch",
    hello: "Xin chào",
    aboutMeText1:
      "Tôi tên là Nguyễn Tiến Hoạch. Tôi là một nhà phát triển phần mềm độc lập hiện đang sống ở Việt Nam. Tôi là nhà phát triển Fullstack. Công cụ chính của tôi là golang, vue và React.",
    aboutMeText2:
      "Ngoài lập trình, tôi thích đọc manga và manhwa (tôi thích nhất là Legend of the Northern Blade).",
    followMeOn: "Bạn có thể theo dõi tôi ở đây",
  },
  projects: {
    projects: "Dự án",
    description:
      "Tôi đã thực hiện rất nhiều dự án nhỏ trong nhiều năm, nhưng đây là những dự án tôi tự hào nhất.",
  },
  articles: {
    articles: "Bài viết",
    description: "Những bài viết mà tôi thấy thú vị.",
  },
  lab: {
    lab: "Thí nghiệm",
    description: "Một số thử nghiệm UI ngẫu nhiên tôi tìm thấy khi rảnh rỗi.",
  },
  navbar: {
    home: "Trang chủ",
    projects: "Dự án",
    articles: "Bài viết",
    lab: "Thí nghiệm",
    selectLanguage: "Chọn ngôn ngữ",
    toggleTheme: "Đổi giao diện",
  },
  cursor: {
    title: "Chú ý!",
    description: "Di chuột qua trang web để  nhìn thấy con trỏ.",
  },
};
